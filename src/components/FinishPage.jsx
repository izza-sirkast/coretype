...
import { calculateAccuracy, calculateGrossWPM, calculateWPM } from '@/library/functionality'
...
function FinishPage({language, difficulty, timeMode, cursorPos, salahKetik, salahKetikKelebihan, setFinish, setCursorPos, setSalahKetik, setSalahKetikKelebihan, setFocusDiv, setTimer, statsOverTime, setStatsOverTime, salahKetikSemuaCount, setSalahKetikSemuaCount}) {
  ...
  let accuracy = calculateAccuracy(cursorPos, salahKetikSemuaCount, salahKetikKelebihan)
    ...
    <div className="w-full max-h-screen min-h-screen gradient-bg">
        ...
        <div className='mx-auto flex justify-between items-start w-10/12 pr-5 mt-5'>
          ...
          <div className='flex flex-col items-center justify-center mt-10 -ml-32 *:mb-12 '>
            ...
            <RestartButton size={"7"} setFinish={setFinish} setCursorPos={setCursorPos} setSalahKetik={setSalahKetik} setSalahKetikKelebihan={setSalahKetikKelebihan} setFocusDiv={setFocusDiv} setTimer={setTimer} setStatsOverTime={setStatsOverTime} setSalahKetikSemuaCount={setSalahKetikSemuaCount} />

            <GrNext className="border border-white text-white rounded-md text-7xl hover:cursor-pointer hover:bg-white hover:bg-opacity-20 transition-all ease-in mr-2" onClick={() => {window.location.reload()}}></GrNext>
          </div>
        </div>
    </div>
  )
